...
namespace Portland
{
	...
	public class FPSWalkerEnhanced : MonoBehaviour
	{
		...
		[Tooltip("Updates slave with relative movement.")]
		public GameObject SlaveTransform;
		...
		private void Update()
		{
			//if (Test.Inst != null && Test.Inst.skyCamera != null)
			//{
			...
			if (m_ToggleRun && m_Grounded && Input.GetKeyDown(KeyCode.LeftShift))
			{
		...
		private void FixedUpdate()
		{
			...
			if (m_Grounded)
			{
				...
				if ((sliding && m_SlideWhenOverSlopeLimit) || (m_SlideOnTaggedObjects && m_Hit.collider.tag == "Slide"))
				{
					...
					m_MoveDirection.Set(hitNormal.x, -hitNormal.y, hitNormal.z);
				...
				{
					m_MoveDirection.Set(inputX * inputModifyFactor, /*-m_AntiBumpFactor*/0, inputY * inputModifyFactor);
			...
			//{
			//	SlaveTransform.transform.position.Set
			//	(
			//		SlaveTransform.transform.position.x + transform.position.x - _lastX,
			//		SlaveTransform.transform.position.y + transform.position.x - _lastY,
			//		SlaveTransform.transform.position.z + transform.position.x - _lastZ
			//	);
			//}
		}
